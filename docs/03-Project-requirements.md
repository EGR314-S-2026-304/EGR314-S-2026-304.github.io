...
|---------------| -----------------------------------|
|    Michael   |                  Exposure and gain are essential to a camera. Each helps the photos in the processing stage and also helps the telescope in general. The system will use some sort of light sensor and a set of small motors to control the camera shutter, which should help with light entering the camera chamber. Will also assume responsibility of Housing by CAD and 3D-Printing if the Stretch goal can be attained.            |
